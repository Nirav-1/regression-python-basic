...
os.chdir("F:\\Data analyst\\Data analyst")

# Read the data
mtcar = pds.read_csv("mtcars.csv",index_col=0)
car = pds.read_csv("mtcars.csv",index_col=0)
...
pds.isna(car_try_regression).mean()

# for % of missing value from entire data
pds.isna(car_try_regression).mean().mean()

# for % of missing values in each row
pds.isna(car_try_regression).mean(axis=1)
...
car_try_regression.hp[(car_try_regression.cyl==4) & (pds.isna(car_try_regression.hp))] = med4


car_try_regression.hp[car_try_regression.cyl==6]
med6 = np.median(car_try_regression.hp[car_try_regression.cyl==6].dropna())
car_try_regression.hp[(car_try_regression.cyl==6) & (pds.isna(car_try_regression.hp))] = med6

car_try_regression.hp[car_try_regression.cyl==8]
med8 = np.median(car_try_regression.hp[car_try_regression.cyl==8].dropna())
car_try_regression.hp[(car_try_regression.cyl==8) & (pds.isna(car_try_regression.hp))] = med8
...
###### Regression Modeling ######
multinomModel = smf.ols('qsec ~ mpg+cyl+disp+drat+wt+C(vs)+C(am)+gear+carb+hp',data=train).fit()
...
actualVsPredcit = pds.DataFrame({'actual':test.qsec,'predicted':predict_values})
